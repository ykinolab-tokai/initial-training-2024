...
1. プログラミングをしていると，ある特定の要素を持つNumPy配列や，要素は何でもいいけど決まった形を持つNumPy配列を作りたいことがよくあります．
以下の要素を持つ$5\times5$のNumPy配列を作成する関数を[公式ドキュメント](https://numpy.org/doc/stable/reference/routines.array-creation.html)からそれぞれ探し、
配列を作成して表示するプログラムを作成してください．

    - すべての要素が0
    - すべての要素が1
    - 対角の要素（0行0列成分から5行5列成分）が1で，他の要素が0
...
# 第6章: 線形代数の基礎
1. $A=\begin{pmatrix} 2 & -1 \\ 1 & 1 \end{pmatrix}, x=\begin{pmatrix} 1 \\ 1 \end{pmatrix}, b=\begin{pmatrix} 1 \\ 2 \end{pmatrix}$のとき，NumPyを用いて$Ax-b$を計算するプログラムを作成してください．
